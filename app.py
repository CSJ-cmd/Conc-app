    ...
    try:
        ...
        if len(readings) < 5:
            ...
            if not valid:
                ...
                # 4. 강도 추정 공식 적용 (단위: MPa)
                # (1) 일본재료학회 (일반)
                f_jsms = (13 * R0 - 184) * 0.098 * age_coeff
                # (2) 일본건축학회 (일반)
                f_aij = (7.3 * R0 + 100) * 0.098 * age_coeff
                # (3) 과학기술부 (고강도)
                f_mst = (15.2 * R0 - 112) * 0.098 * age_coeff
                # (4) 한전기술 (고강도) - 계수 0.1 사용
                f_kepco = (14.29 * R0 - 8.057) * 0.1 * age_coeff
                ...
                result_data = {
                    "구분": ["일본재료학회 (일반)", "일본건축학회 (일반)", "과학기술부 (고강도)", "한전기술 (고강도)"],
                    "추정 강도 (MPa)": [
                        f"{max(0, f_jsms):.2f}",
                        f"{max(0, f_aij):.2f}",
                        f"{max(0, f_mst):.2f}",
                    ...
                    "비고": ["식: 13R-184", "식: 7.3R+100", "식: 15.2R-112", "식: 14.29R-8.057"]
                }
